...
const UpdateVendor = () => {
  ...
  const { updateVendor } = useUser();

  //   console.log(updateVendor._id);

  useEffect(() => {
    const formattedDate = updateVendor.date
      ? updateVendor.date.split("T")[0]
      : "";
    if (updateVendor) {
      setFormData({
        vendorId: updateVendor.vendorId || "",
        vendorName: updateVendor.vendorName || "",
        contactName: updateVendor.contactName || "",
        date: formattedDate,
        alternateName: updateVendor.alternateName || "",
        contactEmail: updateVendor.contactEmail || "",
        alternateEmail: updateVendor.alternateEmail || "",
        productService: updateVendor.productService || "",
        category: updateVendor.category || "",
        productManufacture: updateVendor.productManufacture || "",
        additionalInfo: updateVendor.additionalInfo || "",
        bankName: updateVendor.bankName || "",
        branchName: updateVendor.branchName || "",
        ifscCode: updateVendor.ifscCode || "",
        bankingName: updateVendor.bankingName || "",
      });
    }
  }, [updateVendor]); // This effect runs whenever `updateVendor` is updated
  ...
  const handleFormVendor = async (e) => {
    ...
    try {
      const res = await axios.put(
        `https://erp-inventery-node-js.onrender.com/vendor/update/${updateVendor._id}`,
        formData
      );
